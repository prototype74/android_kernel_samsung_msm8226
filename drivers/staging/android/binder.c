...
static int binder_ioctl_set_ctx_mgr(struct file *filp)
{
	...
	uid_t curr_euid = current->cred->euid;
...
static int binder_state_show(struct seq_file *m, void *unused)
{
	...
	hlist_for_each_entry(device, pos1, &binder_devices, hlist) {
		...
		hlist_for_each_entry(node, pos2, &context->binder_dead_nodes,
				     dead_node)
			print_binder_node(m, node);
	...
	hlist_for_each_entry(device, pos1, &binder_devices, hlist) {
		...
		hlist_for_each_entry(proc, pos2, &context->binder_procs, proc_node)
...
static int binder_proc_show(struct seq_file *m, void *unused)
{
	...
	hlist_for_each_entry(device, pos1, &binder_devices, hlist) {
		...
		hlist_for_each_entry(itr, pos2, &context->binder_procs, proc_node) {
			if (itr->pid == pid) {
...
err_init_binder_device_failed:
	hlist_for_each_entry_safe(device, pos, tmp, &binder_devices, hlist) {
